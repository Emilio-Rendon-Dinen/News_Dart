...
class SavedArticles extends HookWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<LocalArticleBloc>()..add(const GetSaavedArticles()),
      child: Scaffold(
        appBar: _buildAppBar(),
        body: _buildBody(),
      ),
  ...
  Widget _buildArticlesList(List<ArticleDO> articles) {
    if (articles.isEmpty) {
      return const Center(
          child: Text(
        'NO SAVED ARTICLES',
  ...
  void _onRemoveArticle(BuildContext context, ArticleDO article) {
